...
def get_question(local_list, repeat_list, api_func, rating=None):
    # If rating is selected, we MUST use the API (local lists don't have ratings)
    if rating:
        use_api = True
    else:
        # If no rating, 50/50 chance
        use_api = random.randint(0, 1) == 1

    if not use_api:
        # Use local list
        if not local_list: 
            print(f"Refilling {local_list.__name__ if hasattr(local_list, '__name__') else 'list'} from repeats...")
            local_list.extend(repeat_list)
            repeat_list.clear()
        ...
        # Use API
        try:
            r1 = api_func(rating) # Pass the rating to the API function
            if not r1: return "API Error."
            return r1
...
class GameButtonView(View):
    def __init__(self, rating=None):
        # We store the rating in the view instance so buttons can access it
        super().__init__(timeout=None)
        self.rating = rating

    def get_question_wrapper(self, local_list, repeat_list, api_func):
        # Pass the stored self.rating to the question generator
        return get_question(local_list, repeat_list, api_func, rating=self.rating)
    ...
    async def truth_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # We use view=self to keep the SAME view (with the same rating) for the new message
...
async def on_ready():
    ...
    # This ensures buttons from OLD messages still work after restart, 
    # though they will default to rating=None (Random)
    bot.add_view(GameButtonView(rating=None)) 
...
async def who(interaction, rating: app_commands.Choice[str] = None):
    ...
    embed=discord.Embed(title=r1, color=0xe5deca)
    embed.set_footer(text=f"Requested by {interaction.user.name}.",icon_url=f"{interaction.user.avatar}")
    await interaction.response.send_message(embed=embed, view=GameButtonView(rating=rating_value))
...
async def td(interaction):
    ...
    embed=discord.Embed(title=r1, color=0xe5deca)
    embed.set_footer(text=f"Requested by {interaction.user.name}.", icon_url=f"{interaction.user.avatar}")
    await interaction.response.send_message(embed=embed, view=GameButtonView(rating=None))
